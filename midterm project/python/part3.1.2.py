...
class Quanser:
    ...
    def residuals_part3(self, images, p, sparse=False,image_number=0):
        N = 7
        M = 5 + 3*N
        L = images
        r = np.zeros((1,2*N*L))
        ...
        m  = p[0:M] #Statics
        angles = p[M:].reshape((L,3)) #All 3 angles over all pictures
 
        for i in range(L):
            #does only the correct image so it can be mapped onto the jacobian
            if sparse:
                i = image_number
...
####ESTIMATE STATICS AND ANGLES FOR ALL IMAGES - Step 2:
p = np.zeros((M + 3*L))
sparse = False # for faster code
#initial conditions:
p[0:M] = m #constant statics as initial estimates as in part 3.1.1
p[M:]  =  trajectory.reshape(3*L) #Use angles/trajectory found in PART1  as initial estimates

#residuals defined:
residualsfun2 = lambda p,sparse,i : quanser.residuals_part3(L, p,sparse=sparse,image_number=i)
p = optimized_levenberg_marquardt(residualsfun2, p,termination_threshold=1e-6)



 
#TODO: CHANGE m in step 3 to parameterized version, also do the generalized coordinates thingy..
###USE ESTIMATED STATICS TO FIND ANGLES (p3) - Step 3:

m  = p[0:M] #estimated statics from step 2
